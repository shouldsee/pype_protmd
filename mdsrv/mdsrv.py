...
def jinja2_render_str(myString, data):
...
app =bp= Blueprint('mdsrv', __name__,
...
last_struct = lst = [None]
@app.route( '/webapp/' )
@app.route( '/webapp/<path:filename>' )
@requires_auth
@crossdomain( origin='*' )
def webapp( filename="index.html",lst=lst):
    if request.args.get('struc'):
        struc = request.args.get('struc')
        struc = struc.lstrip('/').split('/',2)[2]
        lst[0] = os.path.realpath(struc)
    #     global struct
    #     structurefile = struc.split('file://')[1]
    #     path = structurefile.split('/')[0]
    #     name = '/'.join(structurefile.split('/')[1:])
...
def file( root, filename):
    ...
    return send_from_directory( os.path.realpath(directory), filename )
...
def traj_frame( frame, root, filename ):
    ...
    return TRAJ_CACHE.get( os.path.realpath(os.path.join(root,filename)), lst[0] ).get_frame_string(
...
def traj_numframes( root, filename ):
    return str( TRAJ_CACHE.get( os.path.realpath(root+'/'+filename), last_struct[0] ).numframes )
...
def parse_args():
    ...
    parser.add_argument('--prefix',default='/',type=str,help='prefix to mount app under')
...
def main():
    ...
    app.config['BASE_URL'] = args.prefix
