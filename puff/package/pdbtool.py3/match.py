...
import math
import numpy
import vector3d, util, pdbstruct
...
def calculate_superposition_matrix(atoms1, atoms2):
  ...
  def convert_to_matrix3d(numpy_matrix3d):
    ...
    for i in range(3):
      for j in range(3):
        result.set_elem(i, j, numpy_rotation[j, i])
    return result
...
def get_raw_rmsd(pdb1, pdb2, segments1, segments2, atom_types):
  polymer1 = pdbstruct.Polymer(pdb1)
  polymer2 = pdbstruct.Polymer(pdb2)
...
def get_best_alignment(pdb1, pdb2, segments1, segments2, atom_types):
  ...
  center1 = pdbstruct.get_center(atoms1)
  polymer1.transform(vector3d.Translation(-center1))
  polymer2.transform(vector3d.Translation(-pdbstruct.get_center(atoms2)))
...
if __name__ == '__main__':
  ...
  needed if there are no chain identifiers. Put insertions at the end of
  ...
  if '-s' in flags:
    os.system('show.py %s %s > /dev/null &' % (pdb1, temp_pdb))
