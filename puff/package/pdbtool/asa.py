...
def get_is_buried_state(soup, fname='buried', cutoff=0.2):
  "Returns list for each residue where True=is_buried"
  if not os.path.isfile(fname):
    residue_asas = calculate_residue_asas(soup)
    residue_types = [r.type for r in soup.residues()]
    buried = []
    residue_ref_asas = []
    for asa, t in zip(residue_asas, residue_types):
      if t in unfolded_ref_asa:
        ref_asa = unfolded_ref_asa[t]
        buried.append(asa/ref_asa)
        residue_ref_asas.append(ref_asa)
      else:
        buried.append(0.0)
        residue_ref_asas.append(0.0)
    if fname:
      f = open(fname, 'w')
      for i in range(len(residue_asas)):
        res = soup.residue(i)
        name = "%s:%d-%s" % (res.chain_id, res.num, res.type)
        s = "%f %s %f/%f\n" % \
            (buried[i], name, residue_asas[i],
             residue_ref_asas[i])
        f.write(s)
      f.close()
    soup.load_residue_bfactors(buried)
  else:
    buried = util.read_column(fname, 0)
  return [b<=cutoff for b in buried]
